...
function InputTweakData:init(tweak_data)
	...
	local function valid_range(data, var, b, c, ex_b, ex_c)
		...
		if not valid then
			slot11 = "" .. var .. " value is " .. a .. " it should be in the range [" .. b .. ((ex_b and "<") or "<=") .. var .. ((ex_c and "<") or "<=") .. c .. "]"
	...
	valid_range(slot4, self.gamepad, "look_speed_dead_zone", 0, 1, true)

	slot10 = true

	valid_range(slot4, self.gamepad, "look_speed_transition_zone", 0, 1, true)

	slot10 = true

	valid_range(slot4, self.gamepad, "look_speed_transition_occluder", 0, 1, true)

	slot10 = false

	valid_range(slot4, self.gamepad, "aim_assist_gradient_min", 0, 1, false)

	slot10 = false

	valid_range(slot4, self.gamepad, "aim_assist_gradient_max", 0, 1, false)

	slot10 = false

	valid_range(slot4, self.gamepad, "aim_assist_move_th_min", 0, 1, false)

	slot10 = false

	valid_range(slot4, self.gamepad, "aim_assist_move_th_max", 0, 1, false)

	slot5 = "[InputTweakData] Init"
