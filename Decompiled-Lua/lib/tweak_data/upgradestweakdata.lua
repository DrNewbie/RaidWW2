...
function UpgradesTweakData:init()
	...
	self._create_generic_description_data_types(slot2)

	slot3 = self

	self._init_value_tables(slot2)

	slot3 = self

	self._old_init(slot2)

	slot3 = self

	self._create_raid_team_definitions(slot2)

	slot3 = self

	self._create_raid_warcry_definitions(slot2)

	slot3 = self

	self._create_raid_definitions_modifiers(slot2)

	slot3 = self

	self._create_raid_definitions_abilities(slot2)

	slot3 = self

	self._create_weapon_definitions(slot2)

	slot3 = self

	self._create_grenades_definitions(slot2)

	slot3 = self

	self._raid_temporary_definitions(slot2)

	slot3 = self

	self._create_melee_weapon_definitions(slot2)

	slot3 = self

	self._pistol_definitions(slot2)

	slot3 = self

	self._assault_rifle_definitions(slot2)

	slot3 = self

	self._smg_definitions(slot2)

	slot3 = self

	self._shotgun_definitions(slot2)

	slot3 = self

	self._lmg_definitions(slot2)

	slot3 = self

	self._snp_definitions(slot2)
...
function UpgradesTweakData:_create_raid_team_definitions()
	...
	slot9 = self.values.team.player.warcry_damage_multiplier

	self._create_definition_levels(, self, "warcry_team_damage_multiplier", "team", "warcry_damage_multiplier", "player", false)
	...
	slot9 = self.values.team.player.warcry_damage_reduction_multiplier

	self._create_definition_levels(, self, "warcry_team_damage_reduction_multiplier", "team", "warcry_damage_reduction_multiplier", "player", false)
	...
	slot9 = self.values.team.player.warcry_stamina_multiplier

	self._create_definition_levels(, self, "warcry_team_stamina_multiplier", "team", "warcry_stamina_multiplier", "player", false)
	...
	slot9 = self.values.team.player.warcry_health_regeneration

	self._create_definition_levels(, self, "warcry_team_health_regeneration", "team", "warcry_health_regeneration", "player", false)
	...
	slot9 = self.values.team.player.warcry_movement_speed_multiplier

	self._create_definition_levels(, self, "warcry_team_movement_speed_multiplier", "team", "warcry_movement_speed_multiplier", "player", false)
	...
	slot9 = self.values.team.player.warcry_damage_buff_bonus

	self._create_definition_levels(, self, "warcry_team_damage_buff_bonus", "team", "warcry_damage_buff_bonus", "player", true)
	...
	slot9 = self.values.team.player.warcry_team_movement_speed_bonus

	self._create_definition_levels(, self, "warcry_team_movement_speed_bonus", "team", "warcry_team_movement_speed_bonus", "player", true)
	...
	slot9 = self.values.team.player.warcry_team_damage_reduction_bonus

	self._create_definition_levels(, self, "warcry_team_damage_reduction_bonus", "team", "warcry_team_damage_reduction_bonus", "player", true)
	...
	slot9 = self.values.team.player.warcry_team_damage_reduction_bonus_on_activate

	self._create_definition_levels(, self, "warcry_team_damage_reduction_bonus_on_activate", "team", "warcry_team_damage_reduction_bonus_on_activate", "player", true)
...
function UpgradesTweakData:_create_raid_warcry_definitions()
	...
	slot9 = self.values.player.warcry_duration

	self._create_definition_levels(, self, "warcry_player_duration", "feature", "warcry_duration", "player", true)
	...
	self._create_definition(, self, "warcry_player_aim_assist", "feature", "warcry_aim_assist", "player", false)
	...
	self._create_definition(, self, "warcry_player_health_regen_on_kill", "feature", "warcry_health_regen_on_kill", "player", false)
	...
	slot9 = self.values.player.warcry_health_regen_amount

	self._create_definition_levels(, self, "warcry_player_health_regen_amount", "feature", "warcry_health_regen_amount", "player", false)
	...
	self._create_definition(, self, "warcry_player_nullify_spread", "feature", "warcry_nullify_spread", "player", false)
	...
	self._create_definition(, self, "warcry_player_sniper_shoot_through_enemies", "feature", "warcry_sniper_shoot_through_enemies", "player", false)
	...
	self._create_definition(, self, "warcry_player_aim_assist_aim_at_head", "feature", "warcry_aim_assist_aim_at_head", "player", false)
	...
	slot9 = self.values.player.warcry_aim_assist_radius

	self._create_definition_levels(, self, "warcry_player_aim_assist_radius", "feature", "warcry_aim_assist_radius", "player", false)

	self.values.player.warcry_headshot_multiplier_bonus = {
		1.2,
		1.7
	}
	slot9 = self.values.player.warcry_headshot_multiplier_bonus

	self._create_definition_levels(, self, "warcry_player_headshot_multiplier_bonus", "feature", "warcry_headshot_multiplier_bonus", "player", true)

	self.values.player.warcry_long_range_multiplier_bonus = {
		1.2,
		1.7
	}
	slot9 = self.values.player.warcry_long_range_multiplier_bonus

	self._create_definition_levels(, self, "warcry_player_long_range_multiplier_bonus", "feature", "warcry_long_range_multiplier_bonus", "player", true)
	...
	self._create_definition(, self, "warcry_player_no_reloads", "feature", "warcry_no_reloads", "player", false)
	...
	self._create_definition(, self, "warcry_player_refill_clip", "feature", "warcry_refill_clip", "player", false)
	...
	slot9 = self.values.player.warcry_ammo_consumption

	self._create_definition_levels(, self, "warcry_player_ammo_consumption", "feature", "warcry_ammo_consumption", "player", false)
	...
	self._create_definition_levels(, self, "warcry_turret_overheat_multiplier", "feature", "warcry_overheat_multiplier", "player", false)

	self.values.player.warcry_dismemberment_multiplier_bonus = {
		1.2,
		1.7
	}
	slot9 = self.values.player.warcry_dismemberment_multiplier_bonus

	self._create_definition_levels(, self, "warcry_player_dismemberment_multiplier_bonus", "feature", "warcry_dismemberment_multiplier_bonus", "player", true)

	self.values.player.warcry_low_health_multiplier_bonus = {
		1.2,
		1.7
	}
	slot9 = self.values.player.warcry_low_health_multiplier_bonus

	self._create_definition_levels(, self, "warcry_player_low_health_multiplier_bonus", "feature", "warcry_low_health_multiplier_bonus", "player", true)
	...
	slot9 = self.values.player.warcry_team_heal_bonus

	self._create_definition_levels(, self, "warcry_player_team_heal_bonus", "feature", "warcry_team_heal_bonus", "player", true)
	...
	slot9 = self.values.player.warcry_dodge

	self._create_definition_levels(, self, "warcry_player_dodge", "feature", "warcry_dodge", "player", false)
	...
	slot9 = self.values.player.warcry_slower_detection

	self._create_definition_levels(, self, "warcry_player_slower_detection", "feature", "warcry_slower_detection", "player", false)

	self.values.player.warcry_melee_multiplier_bonus = {
		1.2,
		1.7
	}
	slot9 = self.values.player.warcry_melee_multiplier_bonus

	self._create_definition_levels(, self, "warcry_player_melee_multiplier_bonus", "feature", "warcry_melee_multiplier_bonus", "player", true)

	self.values.player.warcry_short_range_multiplier_bonus = {
		1.2,
		1.7
	}
	slot9 = self.values.player.warcry_short_range_multiplier_bonus

	self._create_definition_levels(, self, "warcry_player_short_range_multiplier_bonus", "feature", "warcry_short_range_multiplier_bonus", "player", true)
	...
	slot9 = self.values.player.warcry_grenade_clusters

	self._create_definition_levels(, self, "warcry_player_grenade_clusters", "feature", "warcry_grenade_clusters", "player", false)
	...
	slot9 = self.values.player.warcry_grenade_cluster_range

	self._create_definition_levels(, self, "warcry_player_grenade_cluster_range", "feature", "warcry_grenade_cluster_range", "player", false)
	...
	slot9 = self.values.player.warcry_grenade_cluster_damage

	self._create_definition_levels(, self, "warcry_player_grenade_cluster_damage", "feature", "warcry_grenade_cluster_damage", "player", false)

	self.values.player.warcry_explosions_multiplier_bonus = {
		1.2,
		1.7
	}
	slot9 = self.values.player.warcry_explosions_multiplier_bonus

	self._create_definition_levels(, self, "warcry_player_explosions_multiplier_bonus", "feature", "warcry_explosions_multiplier_bonus", "player", true)

	self.values.player.warcry_killstreak_multiplier_bonus = {
		1.2,
		1.7
	}
	slot9 = self.values.player.warcry_killstreak_multiplier_bonus

	self._create_definition_levels(, self, "warcry_player_killstreak_multiplier_bonus", "feature", "warcry_killstreak_multiplier_bonus", "player", true)
...
function UpgradesTweakData:_create_raid_definitions_modifiers()
	...
	slot10 = self.description_data_types.generic_multiplier

	self._create_definition_levels(, self, "player_max_health_multiplier", "feature", "max_health_multiplier", "player", true, self.values.player.max_health_multiplier)
	...
	self._create_definition_levels(, self, "player_pick_up_ammo_multiplier", "feature", "pick_up_ammo_multiplier", "player", true, self.values.player.pick_up_ammo_multiplier)
	...
	self._create_definition_levels(, self, "player_pick_up_health_multiplier", "feature", "pick_up_health_multiplier", "player", true, self.values.player.pick_up_health_multiplier)
	...
	self._create_definition_levels(, self, "player_primary_ammo_increase", "feature", "primary_ammo_increase", "player", true, self.values.player.primary_ammo_increase)
	...
	self._create_definition_levels(, self, "player_secondary_ammo_increase", "feature", "secondary_ammo_increase", "player", true, self.values.player.secondary_ammo_increase)
	...
	self._create_definition_levels(, self, "player_all_movement_speed_increase", "feature", "all_movement_speed_increase", "player", true, self.values.player.all_movement_speed_increase)
	...
	self._create_definition_levels(, self, "player_run_speed_increase", "feature", "run_speed_increase", "player", true, self.values.player.run_speed_increase)
	...
	slot9 = self.values.player.climb_speed_increase

	self._create_definition_levels(, self, "player_climb_speed_increase", "feature", "climb_speed_increase", "player", true)
	...
	self._create_definition_levels(, self, "player_crouch_speed_increase", "feature", "crouch_speed_increase", "player", true, self.values.player.crouch_speed_increase)
	...
	slot10 = self.description_data_types.generic_multiplier_reductive_string

	self._create_definition_levels(, self, "player_carry_penalty_decrease", "feature", "carry_penalty_decrease", "player", true, self.values.player.carry_penalty_decrease)
	...
	slot10 = self.description_data_types.reductive_multiplier

	self._create_definition_levels(, self, "player_running_damage_reduction", "feature", "running_damage_reduction", "player", true, self.values.player.running_damage_reduction)
	...
	self._create_definition_levels(, self, "player_crouching_damage_reduction", "feature", "crouching_damage_reduction", "player", true, self.values.player.crouching_damage_reduction)
	...
	self._create_definition_levels(, self, "player_interacting_damage_reduction", "feature", "interacting_damage_reduction", "player", true, self.values.player.interacting_damage_reduction)
	...
	self._create_definition_levels(, self, "player_bullet_damage_reduction", "feature", "bullet_damage_reduction", "player", true, self.values.player.bullet_damage_reduction)
	...
	self._create_definition_levels(, self, "player_melee_damage_reduction", "feature", "melee_damage_reduction", "player", true, self.values.player.melee_damage_reduction)
	...
	self._create_definition_levels(, self, "player_headshot_damage_multiplier", "feature", "headshot_damage_multiplier", "player", true, self.values.player.headshot_damage_multiplier)
	...
	self._create_definition_levels(, self, "player_melee_damage_multiplier", "feature", "melee_damage_multiplier", "player", true, self.values.player.melee_damage_multiplier)
	...
	self._create_definition_levels(, self, "player_critical_hit_chance", "feature", "critical_hit_chance", "player", true, self.values.player.critical_hit_chance)
	...
	slot9 = self.values.player.enter_steelsight_speed_multiplier

	self._create_definition_levels(, self, "player_enter_steelsight_speed_multiplier", "feature", "enter_steelsight_speed_multiplier", "player", true)
	...
	self._create_definition_levels(, self, "player_ready_weapon_speed_multiplier", "feature", "ready_weapon_speed_multiplier", "player", true, self.values.player.ready_weapon_speed_multiplier)

	self.values.player.steelsight_speed_increase = {
		1.25
	}
	slot9 = self.values.player.steelsight_speed_increase

	self._create_definition_levels(, self, "player_steelsight_speed_increase", "feature", "steelsight_speed_increase", "player", true)

	self.values.player.weapon_sway_decrease = {
		0.7,
		0.4
	}
	slot9 = self.values.player.weapon_sway_decrease

	self._create_definition_levels(, self, "player_weapon_sway_decrease", "feature", "weapon_sway_decrease", "player", true)
	...
	self._create_definition_levels(, self, "player_on_hit_flinch_reduction", "feature", "on_hit_flinch_reduction", "player", true, self.values.player.on_hit_flinch_reduction)
	...
	self._create_definition_levels(, self, "player_swap_speed_multiplier", "feature", "swap_speed_multiplier", "player", true, self.values.player.swap_speed_multiplier)
	...
	self._create_definition_levels(, self, "interaction_general_interaction_timer_multiplier", "feature", "general_interaction_timer_multiplier", "interaction", true, self.values.interaction.general_interaction_timer_multiplier)

	self.values.interaction.open_crate_multiplier = {
		0.75
	}
	slot9 = self.values.interaction.open_crate_multiplier

	self._create_definition_levels(, self, "interaction_open_crate_multiplier", "feature", "open_crate_multiplier", "interaction", true)
	...
	self._create_definition_levels(, self, "interaction_wheel_hotspot_increase", "feature", "wheel_hotspot_increase", "interaction", true, self.values.interaction.wheel_hotspot_increase)

	self.values.interaction.wheel_amount_decrease = {
		1,
		2
	}
	slot10 = self.description_data_types.raw_value_reduction

	self._create_definition_levels(, self, "interaction_wheel_amount_decrease", "feature", "wheel_amount_decrease", "interaction", true, self.values.interaction.wheel_amount_decrease)
	...
	self._create_definition_levels(, self, "interaction_wheel_rotation_speed_increase", "feature", "wheel_rotation_speed_increase", "interaction", true, self.values.interaction.wheel_rotation_speed_increase)

	self.values.interaction.dynamite_plant_multiplier = {
		0.75
	}
	slot9 = self.values.interaction.dynamite_plant_multiplier

	self._create_definition_levels(, self, "interaction_dynamite_plant_multiplier", "feature", "dynamite_plant_multiplier", "interaction", true)

	self.values.interaction.carry_pickup_multiplier = {
		0.8,
		0.6
	}
	slot9 = self.values.interaction.carry_pickup_multiplier

	self._create_definition_levels(, self, "interaction_carry_pickup_multiplier", "feature", "carry_pickup_multiplier", "interaction", true)
	...
	self._create_definition_levels(, self, "player_stamina_multiplier", "feature", "stamina_multiplier", "player", true, self.values.player.stamina_multiplier)
	...
	self._create_definition_levels(, self, "player_stamina_regeneration_increase", "feature", "stamina_regeneration_increase", "player", true, self.values.player.stamina_regeneration_increase)

	self.values.player.stamina_decay_decrease = {
		0.9
	}
	slot9 = self.values.player.stamina_decay_decrease

	self._create_definition_levels(, self, "player_stamina_decay_decrease", "feature", "stamina_decay_decrease", "player", true)

	self.values.player.stamina_regen_timer_multiplier = {
		0.75,
		0.5
	}
	slot9 = self.values.player.stamina_regen_timer_multiplier

	self._create_definition_levels(, self, "player_stamina_regen_timer_multiplier", "feature", "stamina_regen_timer_multiplier", "player", true)

	self.values.player.revive_from_bleed_out = {
		1,
		3
	}
	slot9 = self.values.player.revive_from_bleed_out

	self._create_definition_levels(, self, "player_revive_from_bleed_out", "feature", "revive_from_bleed_out", "player", true)
	...
	slot10 = self.description_data_types.raw_value_amount

	self._create_definition_levels(, self, "player_bleedout_timer_increase", "feature", "bleedout_timer_increase", "player", true, self.values.player.bleedout_timer_increase)
	...
	self._create_definition_levels(, self, "player_revive_interaction_speed_multiplier", "feature", "revive_interaction_speed_multiplier", "player", true, self.values.player.revive_interaction_speed_multiplier)

	self.values.player.long_dis_revive = {
		1
	}
	slot9 = self.values.player.long_dis_revive

	self._create_definition_levels(, self, "player_long_dis_revive", "feature", "long_dis_revive", "player", true)
...
function UpgradesTweakData:_create_raid_definitions_abilities()
	...
	self._create_definition(, self, "player_highlight_enemy", "feature", "highlight_enemy", "player", true)
	...
	self._create_definition(, self, "player_can_use_scope", "feature", "can_use_scope", "player", true)
	...
	self._create_definition_levels(, self, "player_highlight_enemy_damage_bonus", "feature", "highlight_enemy_damage_bonus", "player", true, self.values.player.highlight_enemy_damage_bonus)
	...
	self._create_definition(, self, "player_undetectable_by_spotters", "feature", "undetectable_by_spotters", "player", true)
	...
	self._create_definition(, self, "player_self_administered_adrenaline", "feature", "self_administered_adrenaline", "player", true)
	...
	self._create_definition(, self, "player_toss_ammo", "feature", "toss_ammo", "player", true)
	...
	self._create_definition(, self, "player_warcry_dmg", "feature", "warcry_dmg", "player", true)
...
function UpgradesTweakData:_raid_temporary_definitions()
	...
	slot9 = self.values.temporary.revived_damage_bonus

	self._create_definition_levels(, self, "temporary_revived_damage_bonus", "temporary", "revived_damage_bonus", "temporary", false)
	...
	slot9 = self.values.temporary.revived_melee_damage_bonus

	self._create_definition_levels(, self, "temporary_revived_melee_damage_bonus", "temporary", "revived_melee_damage_bonus", "temporary", false)
	...
	slot10 = self.description_data_types.temporary_reduction

	self._create_definition_levels(, self, "temporary_on_revived_damage_reduction", "temporary", "on_revived_damage_reduction", "temporary", true, self.values.temporary.on_revived_damage_reduction)
...
function UpgradesTweakData:_create_weapon_definitions()
	...
	self._create_definition(, self, "player_recon_tier_4_unlocked", "feature", "recon_tier_4_unlocked", "player", false)
	...
	self._create_definition(, self, "player_assault_tier_4_unlocked", "feature", "assault_tier_4_unlocked", "player", false)
	...
	self._create_definition(, self, "player_infiltrator_tier_4_unlocked", "feature", "infiltrator_tier_4_unlocked", "player", false)
	...
	self._create_definition(, self, "player_demolitions_tier_4_unlocked", "feature", "demolitions_tier_4_unlocked", "player", false)
	...
	slot9 = self.values.player.weapon_tier_unlocked

	self._create_definition_levels(, self, "player_weapon_tier_unlocked", "feature", "weapon_tier_unlocked", "player", false)
	...
	self._create_definition_levels(, self, "player_grenade_quantity", "feature", "grenade_quantity", "player", false, self.values.player.grenade_quantity)
	...
	self._create_definition_levels(, self, "player_turret_m2_overheat_reduction", "feature", "turret_m2_overheat_reduction", "player", false, self.values.player.turret_m2_overheat_reduction)
	...
	slot9 = self.values.player.turret_flakvierling_overheat_reduction

	self._create_definition_levels(, self, "player_turret_flakvierling_overheat_reduction", "feature", "turret_flakvierling_overheat_reduction", "player", false)
...
function UpgradesTweakData:_primary_weapon_definitions()
	...
	slot9 = self.values.primary_weapon.damage_multiplier

	self._create_definition_levels(, self, "primary_weapon_damage_multiplier", "feature", "damage_multiplier", "primary_weapon", false)
	...
	slot9 = self.values.primary_weapon.recoil_reduction

	self._create_definition_levels(, self, "primary_weapon_recoil_reduction", "feature", "recoil_reduction", "primary_weapon", false)
	...
	self._create_definition_levels(, self, "primary_weapon_reload_speed_multiplier", "feature", "reload_speed_multiplier", "primary_weapon", true, self.values.primary_weapon.reload_speed_multiplier)
	...
	slot9 = self.values.primary_weapon.enter_steelsight_speed_multiplier

	self._create_definition_levels(, self, "primary_weapon_enter_steelsight_speed_multiplier", "feature", "enter_steelsight_speed_multiplier", "primary_weapon", false)
	...
	slot9 = self.values.primary_weapon.spread_multiplier

	self._create_definition_levels(, self, "primary_weapon_spread_multiplier", "feature", "spread_multiplier", "primary_weapon", false)
	...
	slot9 = self.values.primary_weapon.magazine_upgrade

	self._create_definition_levels(, self, "primary_weapon_magazine_upgrade", "feature", "magazine_upgrade", "primary_weapon", false)
...
function UpgradesTweakData:_secondary_weapon_definitions()
	...
	slot9 = self.values.secondary_weapon.damage_multiplier

	self._create_definition_levels(, self, "secondary_weapon_damage_multiplier", "feature", "damage_multiplier", "secondary_weapon", false)
	...
	slot9 = self.values.secondary_weapon.recoil_reduction

	self._create_definition_levels(, self, "secondary_weapon_recoil_reduction", "feature", "recoil_reduction", "secondary_weapon", false)
	...
	self._create_definition_levels(, self, "secondary_weapon_reload_speed_multiplier", "feature", "reload_speed_multiplier", "secondary_weapon", true, self.values.secondary_weapon.reload_speed_multiplier)
	...
	slot9 = self.values.secondary_weapon.enter_steelsight_speed_multiplier

	self._create_definition_levels(, self, "secondary_weapon_enter_steelsight_speed_multiplier", "feature", "enter_steelsight_speed_multiplier", "secondary_weapon", false)
	...
	slot9 = self.values.secondary_weapon.spread_multiplier

	self._create_definition_levels(, self, "secondary_weapon_spread_multiplier", "feature", "spread_multiplier", "secondary_weapon", false)
	...
	slot9 = self.values.secondary_weapon.magazine_upgrade

	self._create_definition_levels(, self, "secondary_weapon_magazine_upgrade", "feature", "magazine_upgrade", "secondary_weapon", false)
...
function UpgradesTweakData:_pistol_definitions()
	...
	slot9 = self.values.pistol.recoil_reduction

	self._create_definition_levels(, self, "pistol_recoil_reduction", "feature", "recoil_reduction", "pistol", false)

	self.values.pistol.reload_speed_multiplier = {
		1.5
	}
	slot9 = self.values.pistol.reload_speed_multiplier

	self._create_definition_levels(, self, "pistol_reload_speed_multiplier", "feature", "reload_speed_multiplier", "pistol", false)
	...
	self._create_definition_levels(, self, "pistol_damage_multiplier", "feature", "damage_multiplier", "pistol", false, self.values.pistol.damage_multiplier)

	self.values.pistol.enter_steelsight_speed_multiplier = {
		2
	}
	slot9 = self.values.pistol.enter_steelsight_speed_multiplier

	self._create_definition_levels(, self, "pistol_enter_steelsight_speed_multiplier", "feature", "enter_steelsight_speed_multiplier", "pistol", false)

	self.values.pistol.spread_multiplier = {
		0.9
	}
	slot9 = self.values.pistol.spread_multiplier

	self._create_definition_levels(, self, "pistol_spread_multiplier", "feature", "spread_multiplier", "pistol", false)

	self.values.pistol.fire_rate_multiplier = {
		2
	}
	slot9 = self.values.pistol.fire_rate_multiplier

	self._create_definition_levels(, self, "pistol_fire_rate_multiplier", "feature", "fire_rate_multiplier", "pistol", false)

	self.values.pistol.exit_run_speed_multiplier = {
		1.25
	}
	slot9 = self.values.pistol.exit_run_speed_multiplier

	self._create_definition_levels(, self, "pistol_exit_run_speed_multiplier", "feature", "exit_run_speed_multiplier", "pistol", false)

	self.values.pistol.hip_fire_spread_multiplier = {
		0.8
	}
	slot9 = self.values.pistol.hip_fire_spread_multiplier

	self._create_definition_levels(, self, "pistol_hip_fire_spread_multiplier", "feature", "hip_fire_spread_multiplier", "pistol", false)

	self.values.pistol.hip_fire_damage_multiplier = {
		1.2
	}
	slot9 = self.values.pistol.hip_fire_damage_multiplier

	self._create_definition_levels(, self, "pistol_hip_fire_damage_multiplier", "feature", "hip_fire_damage_multiplier", "pistol", false)

	self.values.pistol.swap_speed_multiplier = {
		1.5
	}
	slot9 = self.values.pistol.swap_speed_multiplier

	self._create_definition_levels(, self, "pistol_swap_speed_multiplier", "feature", "swap_speed_multiplier", "pistol", false)

	self.values.pistol.stacking_hit_damage_multiplier = {
		0.1
	}
	slot9 = self.values.pistol.stacking_hit_damage_multiplier

	self._create_definition_levels(, self, "pistol_stacking_hit_damage_multiplier", "feature", "stacking_hit_damage_multiplier", "pistol", false)

	self.values.pistol.stacking_hit_expire_t = {
		2,
		8
	}
	slot9 = self.values.pistol.stacking_hit_expire_t

	self._create_definition_levels(, self, "pistol_stacking_hit_expire_t", "feature", "stacking_hit_expire_t", "pistol", false)

	self.values.pistol.damage_addend = {
		1.5
	}
	slot9 = self.values.pistol.damage_addend

	self._create_definition_levels(, self, "pistol_damage_addend", "feature", "damage_addend", "pistol", false)
...
function UpgradesTweakData:_assault_rifle_definitions()
	...
	slot9 = self.values.assault_rifle.recoil_reduction

	self._create_definition_levels(, self, "assault_rifle_recoil_reduction", "feature", "recoil_reduction", "assault_rifle", false)

	self.values.assault_rifle.recoil_multiplier = {
		0.75
	}
	slot9 = self.values.assault_rifle.recoil_multiplier

	self._create_definition_levels(, self, "assault_rifle_recoil_multiplier", "feature", "recoil_multiplier", "assault_rifle", false)

	self.values.assault_rifle.reload_speed_multiplier = {
		1.25
	}
	slot9 = self.values.assault_rifle.reload_speed_multiplier

	self._create_definition_levels(, self, "assault_rifle_reload_speed_multiplier", "feature", "reload_speed_multiplier", "assault_rifle", false)
	...
	self._create_definition_levels(, self, "assault_rifle_damage_multiplier", "feature", "damage_multiplier", "assault_rifle", false, self.values.assault_rifle.damage_multiplier)

	self.values.assault_rifle.enter_steelsight_speed_multiplier = {
		2
	}
	slot9 = self.values.assault_rifle.enter_steelsight_speed_multiplier

	self._create_definition_levels(, self, "assault_rifle_enter_steelsight_speed_multiplier", "feature", "enter_steelsight_speed_multiplier", "assault_rifle", false)

	self.values.assault_rifle.spread_multiplier = {
		0.9
	}
	slot9 = self.values.assault_rifle.spread_multiplier

	self._create_definition_levels(, self, "assault_rifle_spread_multiplier", "feature", "spread_multiplier", "assault_rifle", false)

	self.values.assault_rifle.fire_rate_multiplier = {
		2
	}
	slot9 = self.values.assault_rifle.fire_rate_multiplier

	self._create_definition_levels(, self, "assault_rifle_fire_rate_multiplier", "feature", "fire_rate_multiplier", "assault_rifle", false)

	self.values.assault_rifle.exit_run_speed_multiplier = {
		1.25
	}
	slot9 = self.values.assault_rifle.exit_run_speed_multiplier

	self._create_definition_levels(, self, "assault_rifle_exit_run_speed_multiplier", "feature", "exit_run_speed_multiplier", "assault_rifle", false)

	self.values.assault_rifle.hip_fire_spread_multiplier = {
		0.8
	}
	slot9 = self.values.assault_rifle.hip_fire_spread_multiplier

	self._create_definition_levels(, self, "assault_rifle_hip_fire_spread_multiplier", "feature", "hip_fire_spread_multiplier", "assault_rifle", false)

	self.values.assault_rifle.hip_fire_damage_multiplier = {
		1.2
	}
	slot9 = self.values.assault_rifle.hip_fire_damage_multiplier

	self._create_definition_levels(, self, "assault_rifle_hip_fire_damage_multiplier", "feature", "hip_fire_damage_multiplier", "assault_rifle", false)

	self.values.assault_rifle.swap_speed_multiplier = {
		1.5
	}
	slot9 = self.values.assault_rifle.swap_speed_multiplier

	self._create_definition_levels(, self, "assault_rifle_swap_speed_multiplier", "feature", "swap_speed_multiplier", "assault_rifle", false)

	self.values.assault_rifle.stacking_hit_damage_multiplier = {
		0.1
	}
	slot9 = self.values.assault_rifle.stacking_hit_damage_multiplier

	self._create_definition_levels(, self, "assault_rifle_stacking_hit_damage_multiplier", "feature", "stacking_hit_damage_multiplier", "assault_rifle", false)

	self.values.assault_rifle.stacking_hit_expire_t = {
		2,
		8
	}
	slot9 = self.values.assault_rifle.stacking_hit_expire_t

	self._create_definition_levels(, self, "assault_rifle_stacking_hit_expire_t", "feature", "stacking_hit_expire_t", "assault_rifle", false)

	self.values.assault_rifle.damage_addend = {
		1.5
	}
	slot9 = self.values.assault_rifle.damage_addend

	self._create_definition_levels(, self, "assault_rifle_damage_addend", "feature", "damage_addend", "assault_rifle", false)

	self.values.assault_rifle.move_spread_multiplier = {
		0.5
	}
	slot9 = self.values.assault_rifle.move_spread_multiplier

	self._create_definition_levels(, self, "assault_rifle_move_spread_multiplier", "feature", "move_spread_multiplier", "assault_rifle", false)

	self.values.assault_rifle.zoom_increase = {
		2
	}
	slot9 = self.values.assault_rifle.zoom_increase

	self._create_definition_levels(, self, "assault_rifle_zoom_increase", "feature", "zoom_increase", "assault_rifle", false)
...
function UpgradesTweakData:_lmg_definitions()
	...
	slot9 = self.values.lmg.recoil_reduction

	self._create_definition_levels(, self, "lmg_recoil_reduction", "feature", "recoil_reduction", "lmg", false)

	self.values.lmg.recoil_multiplier = {
		0.75
	}
	slot9 = self.values.lmg.recoil_multiplier

	self._create_definition_levels(, self, "lmg_recoil_multiplier", "feature", "recoil_multiplier", "lmg", false)

	self.values.lmg.reload_speed_multiplier = {
		1.25
	}
	slot9 = self.values.lmg.reload_speed_multiplier

	self._create_definition_levels(, self, "lmg_reload_speed_multiplier", "feature", "reload_speed_multiplier", "lmg", false)
	...
	self._create_definition_levels(, self, "lmg_damage_multiplier", "feature", "damage_multiplier", "lmg", false, self.values.lmg.damage_multiplier)

	self.values.lmg.enter_steelsight_speed_multiplier = {
		2
	}
	slot9 = self.values.lmg.enter_steelsight_speed_multiplier

	self._create_definition_levels(, self, "lmg_enter_steelsight_speed_multiplier", "feature", "enter_steelsight_speed_multiplier", "lmg", false)

	self.values.lmg.spread_multiplier = {
		0.9
	}
	slot9 = self.values.lmg.spread_multiplier

	self._create_definition_levels(, self, "lmg_spread_multiplier", "feature", "spread_multiplier", "lmg", false)

	self.values.lmg.fire_rate_multiplier = {
		2
	}
	slot9 = self.values.lmg.fire_rate_multiplier

	self._create_definition_levels(, self, "lmg_fire_rate_multiplier", "feature", "fire_rate_multiplier", "lmg", false)

	self.values.lmg.exit_run_speed_multiplier = {
		1.25
	}
	slot9 = self.values.lmg.exit_run_speed_multiplier

	self._create_definition_levels(, self, "lmg_exit_run_speed_multiplier", "feature", "exit_run_speed_multiplier", "lmg", false)

	self.values.lmg.hip_fire_spread_multiplier = {
		0.8
	}
	slot9 = self.values.lmg.hip_fire_spread_multiplier

	self._create_definition_levels(, self, "lmg_hip_fire_spread_multiplier", "feature", "hip_fire_spread_multiplier", "lmg", false)

	self.values.lmg.hip_fire_damage_multiplier = {
		1.2
	}
	slot9 = self.values.lmg.hip_fire_damage_multiplier

	self._create_definition_levels(, self, "lmg_hip_fire_damage_multiplier", "feature", "hip_fire_damage_multiplier", "lmg", false)

	self.values.lmg.swap_speed_multiplier = {
		1.5
	}
	slot9 = self.values.lmg.swap_speed_multiplier

	self._create_definition_levels(, self, "lmg_swap_speed_multiplier", "feature", "swap_speed_multiplier", "lmg", false)

	self.values.lmg.stacking_hit_damage_multiplier = {
		0.1
	}
	slot9 = self.values.lmg.stacking_hit_damage_multiplier

	self._create_definition_levels(, self, "lmg_stacking_hit_damage_multiplier", "feature", "stacking_hit_damage_multiplier", "lmg", false)

	self.values.lmg.stacking_hit_expire_t = {
		2,
		8
	}
	slot9 = self.values.lmg.stacking_hit_expire_t

	self._create_definition_levels(, self, "lmg_stacking_hit_expire_t", "feature", "stacking_hit_expire_t", "lmg", false)

	self.values.lmg.damage_addend = {
		1.5
	}
	slot9 = self.values.lmg.damage_addend

	self._create_definition_levels(, self, "lmg_damage_addend", "feature", "damage_addend", "lmg", false)

	self.values.lmg.move_spread_multiplier = {
		0.5
	}
	slot9 = self.values.lmg.move_spread_multiplier

	self._create_definition_levels(, self, "lmg_move_spread_multiplier", "feature", "move_spread_multiplier", "lmg", false)

	self.values.lmg.zoom_increase = {
		2
	}
	slot9 = self.values.lmg.zoom_increase

	self._create_definition_levels(, self, "lmg_zoom_increase", "feature", "zoom_increase", "lmg", false)
...
function UpgradesTweakData:_snp_definitions()
	...
	slot9 = self.values.snp.recoil_reduction

	self._create_definition_levels(, self, "snp_recoil_reduction", "feature", "recoil_reduction", "snp", false)

	self.values.snp.recoil_multiplier = {
		0.75
	}
	slot9 = self.values.snp.recoil_multiplier

	self._create_definition_levels(, self, "snp_recoil_multiplier", "feature", "recoil_multiplier", "snp", false)

	self.values.snp.reload_speed_multiplier = {
		1.25
	}
	slot9 = self.values.snp.reload_speed_multiplier

	self._create_definition_levels(, self, "snp_reload_speed_multiplier", "feature", "reload_speed_multiplier", "snp", false)
	...
	self._create_definition_levels(, self, "snp_damage_multiplier", "feature", "damage_multiplier", "snp", false, self.values.snp.damage_multiplier)

	self.values.snp.enter_steelsight_speed_multiplier = {
		2
	}
	slot9 = self.values.snp.enter_steelsight_speed_multiplier

	self._create_definition_levels(, self, "snp_enter_steelsight_speed_multiplier", "feature", "enter_steelsight_speed_multiplier", "snp", false)

	self.values.snp.spread_multiplier = {
		0.9
	}
	slot9 = self.values.snp.spread_multiplier

	self._create_definition_levels(, self, "snp_spread_multiplier", "feature", "spread_multiplier", "snp", false)

	self.values.snp.fire_rate_multiplier = {
		2
	}
	slot9 = self.values.snp.fire_rate_multiplier

	self._create_definition_levels(, self, "snp_fire_rate_multiplier", "feature", "fire_rate_multiplier", "snp", false)

	self.values.snp.exit_run_speed_multiplier = {
		1.25
	}
	slot9 = self.values.snp.exit_run_speed_multiplier

	self._create_definition_levels(, self, "snp_exit_run_speed_multiplier", "feature", "exit_run_speed_multiplier", "snp", false)

	self.values.snp.hip_fire_spread_multiplier = {
		0.8
	}
	slot9 = self.values.snp.hip_fire_spread_multiplier

	self._create_definition_levels(, self, "snp_hip_fire_spread_multiplier", "feature", "hip_fire_spread_multiplier", "snp", false)

	self.values.snp.hip_fire_damage_multiplier = {
		1.2
	}
	slot9 = self.values.snp.hip_fire_damage_multiplier

	self._create_definition_levels(, self, "snp_hip_fire_damage_multiplier", "feature", "hip_fire_damage_multiplier", "snp", false)

	self.values.snp.swap_speed_multiplier = {
		1.5
	}
	slot9 = self.values.snp.swap_speed_multiplier

	self._create_definition_levels(, self, "snp_swap_speed_multiplier", "feature", "swap_speed_multiplier", "snp", false)

	self.values.snp.stacking_hit_damage_multiplier = {
		0.1
	}
	slot9 = self.values.snp.stacking_hit_damage_multiplier

	self._create_definition_levels(, self, "snp_stacking_hit_damage_multiplier", "feature", "stacking_hit_damage_multiplier", "snp", false)

	self.values.snp.stacking_hit_expire_t = {
		2,
		8
	}
	slot9 = self.values.snp.stacking_hit_expire_t

	self._create_definition_levels(, self, "snp_stacking_hit_expire_t", "feature", "stacking_hit_expire_t", "snp", false)

	self.values.snp.damage_addend = {
		1.5
	}
	slot9 = self.values.snp.damage_addend

	self._create_definition_levels(, self, "snp_damage_addend", "feature", "damage_addend", "snp", false)

	self.values.snp.move_spread_multiplier = {
		0.5
	}
	slot9 = self.values.snp.move_spread_multiplier

	self._create_definition_levels(, self, "snp_move_spread_multiplier", "feature", "move_spread_multiplier", "snp", false)

	self.values.snp.zoom_increase = {
		2
	}
	slot9 = self.values.snp.zoom_increase

	self._create_definition_levels(, self, "snp_zoom_increase", "feature", "zoom_increase", "snp", false)
...
function UpgradesTweakData:_smg_definitions()
	...
	slot9 = self.values.smg.recoil_reduction

	self._create_definition_levels(, self, "smg_recoil_reduction", "feature", "recoil_reduction", "smg", false)

	self.values.smg.recoil_multiplier = {
		0.75
	}
	slot9 = self.values.smg.recoil_multiplier

	self._create_definition_levels(, self, "smg_recoil_multiplier", "feature", "recoil_multiplier", "smg", false)

	self.values.smg.reload_speed_multiplier = {
		1.35
	}
	slot9 = self.values.smg.reload_speed_multiplier

	self._create_definition_levels(, self, "smg_reload_speed_multiplier", "feature", "reload_speed_multiplier", "smg", false)
	...
	self._create_definition_levels(, self, "smg_damage_multiplier", "feature", "damage_multiplier", "smg", false, self.values.smg.damage_multiplier)

	self.values.smg.enter_steelsight_speed_multiplier = {
		2
	}
	slot9 = self.values.smg.enter_steelsight_speed_multiplier

	self._create_definition_levels(, self, "smg_enter_steelsight_speed_multiplier", "feature", "enter_steelsight_speed_multiplier", "smg", false)

	self.values.smg.spread_multiplier = {
		0.9,
		0.8
	}
	slot9 = self.values.smg.spread_multiplier

	self._create_definition_levels(, self, "smg_spread_multiplier", "feature", "spread_multiplier", "smg", false)

	self.values.smg.fire_rate_multiplier = {
		1.2
	}
	slot9 = self.values.smg.fire_rate_multiplier

	self._create_definition_levels(, self, "smg_fire_rate_multiplier", "feature", "fire_rate_multiplier", "smg", false)

	self.values.smg.exit_run_speed_multiplier = {
		1.25
	}
	slot9 = self.values.smg.exit_run_speed_multiplier

	self._create_definition_levels(, self, "smg_exit_run_speed_multiplier", "feature", "exit_run_speed_multiplier", "smg", false)

	self.values.smg.hip_fire_spread_multiplier = {
		0.8
	}
	slot9 = self.values.smg.hip_fire_spread_multiplier

	self._create_definition_levels(, self, "smg_hip_fire_spread_multiplier", "feature", "hip_fire_spread_multiplier", "smg", false)

	self.values.smg.hip_fire_damage_multiplier = {
		1.2
	}
	slot9 = self.values.smg.hip_fire_damage_multiplier

	self._create_definition_levels(, self, "smg_hip_fire_damage_multiplier", "feature", "hip_fire_damage_multiplier", "smg", false)

	self.values.smg.swap_speed_multiplier = {
		1.5
	}
	slot9 = self.values.smg.swap_speed_multiplier

	self._create_definition_levels(, self, "smg_swap_speed_multiplier", "feature", "swap_speed_multiplier", "smg", false)

	self.values.smg.stacking_hit_damage_multiplier = {
		0.1
	}
	slot9 = self.values.smg.stacking_hit_damage_multiplier

	self._create_definition_levels(, self, "smg_stacking_hit_damage_multiplier", "feature", "stacking_hit_damage_multiplier", "smg", false)

	self.values.smg.stacking_hit_expire_t = {
		2,
		8
	}
	slot9 = self.values.smg.stacking_hit_expire_t

	self._create_definition_levels(, self, "smg_stacking_hit_expire_t", "feature", "stacking_hit_expire_t", "smg", false)

	self.values.smg.damage_addend = {
		1.5
	}
	slot9 = self.values.smg.damage_addend

	self._create_definition_levels(, self, "smg_damage_addend", "feature", "damage_addend", "smg", false)

	self.values.smg.move_spread_multiplier = {
		0.5
	}
	slot9 = self.values.smg.move_spread_multiplier

	self._create_definition_levels(, self, "smg_move_spread_multiplier", "feature", "move_spread_multiplier", "smg", false)

	self.values.smg.zoom_increase = {
		2
	}
	slot9 = self.values.smg.zoom_increase

	self._create_definition_levels(, self, "smg_zoom_increase", "feature", "zoom_increase", "smg", false)
...
function UpgradesTweakData:_shotgun_definitions()
	...
	slot9 = self.values.shotgun.recoil_reduction

	self._create_definition_levels(, self, "shotgun_recoil_reduction", "feature", "recoil_reduction", "shotgun", false)

	self.values.shotgun.recoil_multiplier = {
		0.75
	}
	slot9 = self.values.shotgun.recoil_multiplier

	self._create_definition_levels(, self, "shotgun_recoil_multiplier", "feature", "recoil_multiplier", "shotgun", false)

	self.values.shotgun.reload_speed_multiplier = {
		1.5
	}
	slot9 = self.values.shotgun.reload_speed_multiplier

	self._create_definition_levels(, self, "shotgun_reload_speed_multiplier", "feature", "reload_speed_multiplier", "shotgun", false)
	...
	self._create_definition_levels(, self, "shotgun_damage_multiplier", "feature", "damage_multiplier", "shotgun", false, self.values.shotgun.damage_multiplier)

	self.values.shotgun.enter_steelsight_speed_multiplier = {
		2
	}
	slot9 = self.values.shotgun.enter_steelsight_speed_multiplier

	self._create_definition_levels(, self, "shotgun_enter_steelsight_speed_multiplier", "feature", "enter_steelsight_speed_multiplier", "shotgun", false)

	self.values.shotgun.spread_multiplier = {
		0.9
	}
	slot9 = self.values.shotgun.spread_multiplier

	self._create_definition_levels(, self, "shotgun_spread_multiplier", "feature", "spread_multiplier", "shotgun", false)

	self.values.shotgun.fire_rate_multiplier = {
		1.2
	}
	slot9 = self.values.shotgun.fire_rate_multiplier

	self._create_definition_levels(, self, "shotgun_fire_rate_multiplier", "feature", "fire_rate_multiplier", "shotgun", false)

	self.values.shotgun.exit_run_speed_multiplier = {
		1.25
	}
	slot9 = self.values.shotgun.exit_run_speed_multiplier

	self._create_definition_levels(, self, "shotgun_exit_run_speed_multiplier", "feature", "exit_run_speed_multiplier", "shotgun", false)

	self.values.shotgun.hip_fire_spread_multiplier = {
		0.8
	}
	slot9 = self.values.shotgun.hip_fire_spread_multiplier

	self._create_definition_levels(, self, "shotgun_hip_fire_spread_multiplier", "feature", "hip_fire_spread_multiplier", "shotgun", false)

	self.values.shotgun.hip_fire_damage_multiplier = {
		1.2
	}
	slot9 = self.values.shotgun.hip_fire_damage_multiplier

	self._create_definition_levels(, self, "shotgun_hip_fire_damage_multiplier", "feature", "hip_fire_damage_multiplier", "shotgun", false)

	self.values.shotgun.swap_speed_multiplier = {
		1.5
	}
	slot9 = self.values.shotgun.swap_speed_multiplier

	self._create_definition_levels(, self, "shotgun_swap_speed_multiplier", "feature", "swap_speed_multiplier", "shotgun", false)

	self.values.shotgun.stacking_hit_damage_multiplier = {
		0.1
	}
	slot9 = self.values.shotgun.stacking_hit_damage_multiplier

	self._create_definition_levels(, self, "shotgun_stacking_hit_damage_multiplier", "feature", "stacking_hit_damage_multiplier", "shotgun", false)

	self.values.shotgun.stacking_hit_expire_t = {
		2,
		8
	}
	slot9 = self.values.shotgun.stacking_hit_expire_t

	self._create_definition_levels(, self, "shotgun_stacking_hit_expire_t", "feature", "stacking_hit_expire_t", "shotgun", false)

	self.values.shotgun.damage_addend = {
		1.5
	}
	slot9 = self.values.shotgun.damage_addend

	self._create_definition_levels(, self, "shotgun_damage_addend", "feature", "damage_addend", "shotgun", false)

	self.values.shotgun.move_spread_multiplier = {
		0.5
	}
	slot9 = self.values.shotgun.move_spread_multiplier

	self._create_definition_levels(, self, "shotgun_move_spread_multiplier", "feature", "move_spread_multiplier", "shotgun", false)

	self.values.shotgun.zoom_increase = {
		2
	}
	slot9 = self.values.shotgun.zoom_increase

	self._create_definition_levels(, self, "shotgun_zoom_increase", "feature", "zoom_increase", "shotgun", false)

	self.values.shotgun.consume_no_ammo_chance = {
		0.01,
		0.03
	}
	slot9 = self.values.shotgun.consume_no_ammo_chance

	self._create_definition_levels(, self, "shotgun_consume_no_ammo_chance", "feature", "consume_no_ammo_chance", "shotgun", false)

	self.values.shotgun.melee_knockdown_mul = {
		1.75
	}
	slot9 = self.values.shotgun.melee_knockdown_mul

	self._create_definition_levels(, self, "shotgun_melee_knockdown_mul", "feature", "melee_knockdown_mul", "shotgun", false)
...
function UpgradesTweakData:_create_definition_levels(definition_name, definition_category, upgrade_name, upgrade_category, incremental, values, description_data)
	for index = 1, #values, 1 do
		...
		slot24 = description_data

		self._create_definition(slot15, self, definition_name_level, definition_category, upgrade_name, upgrade_category, incremental, index, true)
	end
...
function UpgradesTweakData:_init_pd2_values()
	...
	slot3 = self.loose_ammo_restore_health_values

	for i, data in ipairs("loose_ammo_restore_health") do
		...
		table.insert("insert", self.values.temporary.loose_ammo_restore_health)
	end
		...
		for index, skill_version in ipairs() do
			...
			for i, desc in ipairs(#skill_version) do
				...
				if i == 1 then
					...
					slot22 = i
					slot20 = tostring(slot21)
				end

				slot18[slot19 .. slot20] = desc
			end
		end
	end
...
function UpgradesTweakData:_old_init()
	...
	self.levels = {}
	slot3 = self.definitions

	for name, upgrade in pairs() do
		...
		if upgrade.prio and upgrade.prio == "high" then
			...
			table.insert(slot8, self.levels[unlock_lvl], 1)
		else
			slot10 = name

			table.insert(slot8, self.levels[unlock_lvl])
		end
	end
		...
		if upgrade.tree then
			if upgrade.step then
				if self.progress[upgrade.tree][upgrade.step] then
					...
					Application.error(slot7, Application, "upgrade collision", upgrade.tree, upgrade.step, self.progress[upgrade.tree][upgrade.step])
				end

				self.progress[upgrade.tree][upgrade.step] = name
			else
				slot10 = "is in no step"

				print(slot7, name, upgrade.tree)
			end
		end
	end
...
function UpgradesTweakData:_player_definitions()
	...
	slot3 = self.values.player.thick_skin
	...
	slot3 = self.values.player.extra_ammo_multiplier
	...
	slot3 = self.values.player.perk_armor_regen_timer_multiplier

	for i, value in ipairs("melee_stacking_hit_expire_t") do
		slot10 = i
		self.definitions["player_perk_armor_regen_timer_multiplier_" .. tostring(slot9)] = {
	...
	slot3 = self.values.player.perk_armor_loss_multiplier

	for i, value in ipairs(slot2) do
		slot10 = i
		self.definitions["player_perk_armor_loss_multiplier_" .. tostring(slot9)] = {
	...
	slot3 = self.values.player.toolset

	for i, _ in ipairs("toolset") do
...
function UpgradesTweakData:_ammo_bag_definitions()
	...
	slot3 = self.values.ammo_bag.ammo_increase

	for i, _ in ipairs() do
		local depends_on = (0 < i - 1 and "ammo_bag_ammo_increase" .. i - 1) or "ammo_bag"
		local unlock_lvl = 11
		local prio = i == 1 and "high"
...
function UpgradesTweakData:_doctor_bag_definitions()
	...
	slot3 = self.values.doctor_bag.amount_increase

	for i, _ in ipairs() do
		local depends_on = (0 < i - 1 and "doctor_bag_amount_increase" .. i - 1) or "doctor_bag"
		local unlock_lvl = 3
		local prio = i == 1 and "high"
...
function UpgradesTweakData:_cable_tie_definitions()
	...
	slot3 = self.values.extra_cable_tie.quantity
...
function UpgradesTweakData:_rep_definitions()
	...
	slot4 = rep_upgrades.classes

	for index, rep_class in ipairs(slot3) do
		for i = 1, 10, 1 do
