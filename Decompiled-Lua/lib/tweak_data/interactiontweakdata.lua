...
function InteractionTweakData:init()
	...
	slot3 = self.key_double
	self.key = deep_clone(100)
	self.key.axis = "x"
	self.numpad = {
		icon = "equipment_bank_manager_key",
	...
	slot3 = self.take_weapons
	self.take_weapons_not_active = deep_clone(120)
	...
	slot3 = self.pick_lock_hard_no_skill
	self.pick_lock_deposit_transport = deep_clone("bar_pick_lock_finished")
	...
	slot5 = "Spine"
	self.hostage_move.interaction_obj = Idstring(slot4)
	self.hostage_stay = {
		icon = "interaction_trade",
	...
	slot5 = "Spine2"
	self.hostage_stay.interaction_obj = Idstring(slot4)
	self.trip_mine = {
	...
	slot4 = self.invisible_interaction_open
	self.fork_lift_sound = deep_clone(self.health_bag.blocked_hint_sound)
	self.fork_lift_sound.text_id = "hud_int_fork_lift_sound"
	slot4 = self.invisible_interaction_open
	self.money_briefcase = deep_clone(self.health_bag.blocked_hint_sound)
	self.money_briefcase.axis = "x"
	slot4 = self.invisible_interaction_open
	self.grenade_briefcase = deep_clone(self.health_bag.blocked_hint_sound)
	self.grenade_briefcase.contour = "deployable"
	slot4 = self.invisible_interaction_open
	self.cash_register = deep_clone(self.health_bag.blocked_hint_sound)
	self.cash_register.axis = "x"
	self.cash_register.interact_distance = 110
	slot4 = self.invisible_interaction_open
	self.atm_interaction = deep_clone(self.health_bag.blocked_hint_sound)
	self.atm_interaction.start_active = false
	self.atm_interaction.contour = "interactable_icon"
	slot4 = self.invisible_interaction_open
	self.weapon_case = deep_clone(self.health_bag.blocked_hint_sound)
	self.weapon_case.axis = "x"
	self.weapon_case.interact_distance = 110
	slot4 = self.weapon_case
	self.weapon_case_close = deep_clone(self.health_bag.blocked_hint_sound)
	self.weapon_case_close.text_id = "hud_int_invisible_interaction_close"
	slot4 = self.invisible_interaction_open
	self.invisible_interaction_close = deep_clone(self.health_bag.blocked_hint_sound)
	...
	slot4 = self.circuit_breaker
	self.hold_circuit_breaker = deep_clone(self.health_bag.blocked_hint_sound)
	...
	slot4 = self.diamond_pickup
	self.safe_loot_pickup = deep_clone()
	self.safe_loot_pickup.start_active = true
	self.safe_loot_pickup.text_id = "hud_int_take"
	slot4 = self.diamond_pickup
	self.mus_pku_artifact = deep_clone()
	...
	slot4 = self.c4
	self.c4_mission_door = deep_clone("money_grab")
	...
	slot4 = self.gold_pile
	self.gold_pile_inactive = deep_clone()
	...
	slot4 = self.intimidate
	self.intimidate_with_contour = deep_clone(true)
	self.intimidate_with_contour.no_contour = false
	slot4 = self.intimidate_and_search
	self.intimidate_and_search_with_contour = deep_clone(false)
	...
	slot4 = self.shaped_sharge
	self.shaped_charge_single = deep_clone(true)
	...
	slot4 = self.stash_planks
	self.barricade_fence = deep_clone(true)
	...
	slot4 = self.pickup_phone
	self.pickup_tablet = deep_clone(true)
	...
	slot4 = self.hold_take_server
	self.hold_take_server_axis = deep_clone(true)
	...
	slot4 = self.money_wrap
	self.money_luggage = deep_clone()
	...
	slot4 = self.gen_pku_artifact_statue
	self.gen_pku_artifact = deep_clone(true)
	...
	slot4 = self.gen_pku_artifact_statue
	self.gen_pku_artifact_painting = deep_clone(true)
	...
	slot4 = self.crate_loot
	self.crate_loot_crowbar = deep_clone(true)
	...
	slot4 = self.weapon_case
	self.weapon_case_not_active = deep_clone(true)
	self.weapon_case_not_active.start_active = false
	slot4 = self.weapon_case
	self.crate_weapon_crowbar = deep_clone(true)
	...
	slot4 = self.invisible_interaction_open
	self.money_scanner = deep_clone(true)
	self.money_scanner.axis = "y"
	slot4 = self.money_wrap
	self.money_small = deep_clone(true)
	...
	slot4 = self.money_small
	self.money_small_take = deep_clone(true)
	...
	slot4 = self.unlock_car_01
	self.unlock_car_02 = deep_clone(true)
	...
	slot4 = self.unlock_car_01
	self.unlock_car_03 = deep_clone(true)
	...
	slot4 = self.security_station
	self.rewire_timelock = deep_clone(true)
	...
	slot4 = self.pick_lock_hard_no_skill
	self.pick_lock_x_axis = deep_clone(true)
	self.pick_lock_x_axis.axis = "x"
	slot4 = self.money_wrap_single_bundle
	self.money_wrap_single_bundle_active = deep_clone(true)
	...
	slot4 = self.doctor_bag
	self.firstaid_box = deep_clone(true)
	...
	slot4 = self.gen_pku_cocaine
	self.gen_pku_cocaine_pure = deep_clone(true)
	...
	slot4 = self.c4_bag
	self.c4_bag_dynamic = deep_clone(true)
	self.c4_bag_dynamic.force_update_position = true
	slot4 = self.shape_charge_plantable
	self.shape_charge_plantable_c4_1 = deep_clone(true)
	self.shape_charge_plantable_c4_1.special_equipment = "c4_1"
	slot4 = self.shape_charge_plantable
	self.shape_charge_plantable_c4_x1 = deep_clone(true)
	...
	slot4 = self.c4_mission_door
	self.c4_x10 = deep_clone(true)
	self.c4_x10.special_equipment = "c4_x10"
	self.c4_x10.axis = "z"
	slot4 = self.pick_lock_hard_no_skill
	self.pick_lock_hard_no_skill_deactivated = deep_clone(true)
	...
	slot4 = self.money_wrap
	self.money_wrap_updating = deep_clone(true)
	...
	slot4 = self.carry_drop_gold
	self.carry_drop_flak_shell = deep_clone(true)
	...
	slot4 = self.carry_drop_gold
	self.carry_drop_tank_shell = deep_clone(true)
	...
	slot4 = self.carry_drop_gold
	self.carry_drop_barrel = deep_clone(true)
	...
	slot4 = self.dynamite_x1_pku
	self.dynamite_x4_pku = deep_clone(true)
	self.dynamite_x4_pku.special_equipment_block = "dynamite_x4"
	slot4 = self.dynamite_x1_pku
	self.dynamite_x5_pku = deep_clone(true)
	...
	slot4 = self.plant_dynamite
	self.plant_dynamite_x5 = deep_clone()
	self.plant_dynamite_x5.special_equipment = "dynamite_x5"
	slot4 = self.plant_dynamite
	self.plant_dynamite_x4 = deep_clone()
	self.plant_dynamite_x4.special_equipment = "dynamite_x4"
	slot4 = self.plant_dynamite
	self.plant_dynamite_from_bag = deep_clone()
	...
	slot4 = self.anwser_radio
	self.tune_radio = deep_clone("cvy_pick_up_thermite")
	...
	slot5 = 0
	local com_wheel_color = Color("turret_pick_up_stop", 1, 0.8)

	local function com_wheel_clbk(say_target_id, default_say_id, post_prefix, past_prefix)
		slot6 = managers.network
		local character = managers.network.session(slot5).local_peer(slot5)._character
		slot7 = character
		local nationality = CriminalsManager.comm_wheel_callout_from_nationality(managers.network.session(slot5))
		local snd = post_prefix .. nationality .. past_prefix
		local text = nil
		slot10 = managers.player
		local player = managers.player.player_unit(slot9)
		...
		slot11 = player
		slot11 = player.movement(slot10)
		slot11 = player.movement(slot10).current_state(slot10)
		local target, target_nationality = player.movement(slot10).current_state(slot10).teammate_aimed_at_by_player(slot10)

		if target ~= nil then
			slot13 = target_nationality
			local target_char = CriminalsManager.comm_wheel_callout_from_nationality(slot12)
			...
			text = managers.localization.text(slot13, managers.localization, say_target_id)
			slot14 = managers.dialog

			if not managers.dialog.is_dialogue_playing_for_local_player(slot13) then
				slot14 = managers.player

				managers.player.stop_all_speaking_except_dialog(slot13)

				slot14 = managers.player
				slot14 = managers.player.player_unit(slot13)
				slot17 = true

				managers.player.player_unit(slot13).sound(slot13).say(slot13, managers.player.player_unit(slot13).sound(slot13), nationality .. "_call_" .. target_char, nil)

				slot14 = managers.player
				slot14 = managers.player.player_unit(slot13)
				slot18 = true

				managers.player.player_unit(slot13).sound(slot13).queue_sound(slot13, managers.player.player_unit(slot13).sound(slot13), "comm_wheel", post_prefix .. nationality .. past_prefix, nil)
			end
		else
			slot14 = default_say_id
			text = managers.localization.text(slot12, managers.localization)
			slot13 = managers.dialog

			if not managers.dialog.is_dialogue_playing_for_local_player(slot12) then
				slot13 = managers.player

				managers.player.stop_all_speaking_except_dialog(slot12)

				slot13 = managers.player
				slot13 = managers.player.player_unit(slot12)
				slot16 = true

				managers.player.player_unit(slot12).sound(slot12).say(slot12, managers.player.player_unit(slot12).sound(slot12), post_prefix .. nationality .. past_prefix, false)
			end
		end

		slot16 = text

		managers.chat.send_message(slot12, managers.chat, ChatManager.GAME, "Player")
	...
	slot6 = self.take_flak_shell
	self.take_flak_shell_bag = deep_clone(true)
	...
	slot6 = self.take_plank
	self.take_plank_bag = deep_clone(true)
	...
	slot6 = self.take_painting
	self.take_painting_active = deep_clone()
	...
	slot6 = self.open_cargo_door
	self.lockpick_cargo_door = deep_clone(true)
	...
	slot6 = self.hold_couple_wagon
	self.hold_decouple_wagon = deep_clone()
	...
	slot6 = self.take_code_book
	self.take_code_book_active = deep_clone(true)
	...
	slot6 = self.hold_take_canister
	self.hold_take_empty_canister = deep_clone(true)
	...
	slot6 = self.hold_fill_jeep
	self.hold_fill_barrel_gasoline = deep_clone(true)
	...
	slot6 = self.sii_tune_radio
	self.sii_play_recordings = deep_clone(true)
	...
	slot6 = self.sii_tune_radio_easy
	self.sii_play_recordings_easy = deep_clone(true)
	...
	slot6 = self.sii_tune_radio_medium
	self.sii_play_recordings_medium = deep_clone(true)
	...
	slot6 = self.sii_play_recordings
	self.sii_replay_last_message = deep_clone(true)
	...
	slot6 = self.sii_play_recordings_easy
	self.sii_replay_last_message_easy = deep_clone(true)
	...
	slot6 = self.sii_play_recordings_medium
	self.sii_replay_last_message_medium = deep_clone(true)
	...
	slot6 = self.hold_contact_mrs_white
	self.hold_contact_boat_driver = deep_clone(true)
	...
	slot6 = self.hold_contact_mrs_white
	self.hold_request_dynamite_airdrop = deep_clone(true)
	...
	slot6 = self.carry_drop
	self.carry_drop_spy = deep_clone(true)
	...
	slot6 = self.sii_play_recordings
	self.sii_change_channel = deep_clone(true)
