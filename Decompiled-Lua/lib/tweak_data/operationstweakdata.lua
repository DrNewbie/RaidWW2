...
function OperationsTweakData:init()
	...
	self._init_loading_screens(slot2)

	slot3 = self

	self._init_regions(slot2)

	slot3 = self

	self._init_raids(slot2)

	slot3 = self

	self._init_operations(slot2)

	slot3 = self

	self._init_progression_data(slot2)

	slot3 = self

	self._init_consumable_missions_data(slot2)
...
function OperationsTweakData:_init_progression_data()
	...
	slot3 = OperationsTweakData.PROGRESSION_GROUPS

	for index, group in pairs() do
		self.progression.mission_groups[group] = {}
		slot8 = self.missions

		for mission_name, mission_data in pairs() do
			if mission_data.progression_groups then
				for j = 1, #mission_data.progression_groups, 1 do
					if mission_data.progression_groups[j] == group then
						...
						table.insert(slot16, self.progression.mission_groups[group])
...
function OperationsTweakData:mission_data(mission_id)
	...
	slot4 = self.missions[mission_id]
	local res = deep_clone(slot3)
...
function OperationsTweakData:get_region_index_from_name(region_name)
	...
	for index, reg_name in ipairs(slot3) do
...
function OperationsTweakData:randomize_operation(operation_id)
	...
	for _, value in ipairs(slot6) do
		slot14 = #value
		local index = math.floor(math.rand(slot13)) + 1
		slot14 = value[index]

		table.insert(math.rand, calculated_index)
	end

	operation.events_index = calculated_index
	slot9 = operation_id
	slot12 = calculated_index

	Application.debug(slot6, Application, "[OperationsTweakData:randomize_operation]", inspect(slot11))
...
function OperationsTweakData:get_operation_indexes_delimited(operation_id)
	...
	return table.concat(slot3, self.missions[operation_id].events_index)
end
function OperationsTweakData:set_operation_indexes_delimited(operation_id, delimited_string)
	slot7 = "|"
	self.missions[operation_id].events_index = string.split(slot5, delimited_string)
