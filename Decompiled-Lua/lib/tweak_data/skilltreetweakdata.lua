...
function SkillTreeTweakData:init(tweak_data)
	...
	self._init_classes(slot3, self)

	slot4 = self

	self._init_skill_list(slot3)

	self.skill_trees = {}
	slot4 = self

	self._init_recon_skill_tree(slot3)

	slot4 = self

	self._init_assault_skill_tree(slot3)

	slot4 = self

	self._init_infiltrator_skill_tree(slot3)

	slot4 = self

	self._init_demolitions_skill_tree(slot3)

	self.automatic_unlock_progressions = {}
	self.default_weapons = {}
	slot4 = self

	self._init_recon_unlock_progression(slot3)

	slot4 = self

	self._init_assault_unlock_progression(slot3)

	slot4 = self

	self._init_infiltrator_unlock_progression(slot3)

	slot4 = self

	self._init_demolitions_unlock_progression(slot3)

	slot4 = self

	self._create_class_warcry_data(slot3)
...
function SkillTreeTweakData:_create_class_warcry_data()
	...
	for class_name, class_data in pairs(slot2) do
		self.class_warcry_data[class_name] = {}
		slot8 = self.skill_trees[class_name]

		for level, skilltree_data in pairs() do
			slot13 = skilltree_data

			for _, skill_data in pairs(slot12) do
				local aquires_field = self.skills[skill_data.skill_name].acquires
				slot19 = aquires_field

				for _, acquires_item in pairs(slot18) do
...
function SkillTreeTweakData:get_weapon_unlock_levels()
	local ret = {}
	slot4 = self.automatic_unlock_progressions

	for class_name, class_unlock_data in pairs(slot3) do
		slot9 = class_unlock_data

		for level, unlock_data in pairs(slot8) do
			if unlock_data.weapons then
				slot14 = unlock_data.weapons

				for _, weapon_unlock in pairs(slot13) do
					slot19 = self.skills[weapon_unlock].upgrades

					for _, upgrade in ipairs(slot18) do
...
function SkillTreeTweakData:get_weapon_unlock_level(weapon_id, class_name)
	slot5 = self.automatic_unlock_progressions[class_name]

	for level, unlock_data in pairs(slot4) do
		if unlock_data.weapons then
			slot10 = unlock_data.weapons

			for _, weapon_unlock in pairs(slot9) do
				slot15 = self.skills[weapon_unlock].upgrades

				for _, upgrade in ipairs(slot14) do
