...
function CarryTweakData:init(tweak_data)
	...
	slot5 = self.types.very_heavy
	self.types.slightly_very_heavy = deep_clone(slot4)
	...
	slot5 = self.types.medium
	self.types.explosives = deep_clone(slot4)
	...
	slot4 = self.flak_shell
	self.flak_shell_explosive = deep_clone("carry_flak_shell")
	...
	slot4 = self.flak_shell
	self.flak_shell_shot_explosive = deep_clone(true)
	...
	slot4 = self.flak_shell
	self.tank_shell_explosive = deep_clone(true)
...
function CarryTweakData:get_carry_ids()
	local t = {}
	slot4 = tweak_data.carry

	for id, _ in pairs(slot3) do
		slot9 = tweak_data.carry[id]

		if type(slot8) == "table" and tweak_data.carry[id].type then
			slot10 = id
