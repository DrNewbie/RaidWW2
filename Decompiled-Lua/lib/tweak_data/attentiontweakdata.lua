...
function AttentionTweakData:init()
	...
	self._init_player(slot2)

	slot3 = self

	self._init_team_AI(slot2)

	slot3 = self

	self._init_civilian(slot2)

	slot3 = self

	self._init_enemy(slot2)

	slot3 = self

	self._init_drill(slot2)

	slot3 = self

	self._init_sentry_gun(slot2)

	slot3 = self

	self._init_prop(slot2)

	slot3 = self

	self._init_custom(slot2)

	slot3 = self

	self._init_distraction_rock(slot2)

	slot3 = self

	self._post_init(slot2)
...
function AttentionTweakData:_init_player()
	...
	slot4 = self.settings.pl_foe_combatant_cbt_stand
	self.settings.pl_foe_non_combatant_cbt_stand = deep_clone()
...
function AttentionTweakData:get_attention_index(setting_name)
	...
	for i_setting, test_setting_name in ipairs(slot3) do
...
function AttentionTweakData:_post_init()
	...
	for setting_name, setting in pairs(slot2) do
		local i_insert = 1
		...
		table.insert(slot8, self.indexes, i_insert)
	end
