...
function LevelsTweakData:get_level_name_from_world_name(world_name)
	...
	for _, entry_name in ipairs(slot3) do
		if world_name == self[entry_name].world_name then
			return entry_name
		end
	end
...
function LevelsTweakData:get_localized_level_name_from_world_name(world_name)
	...
	for _, entry_name in ipairs(slot3) do
		if world_name == self[entry_name].world_name then
			slot10 = self[entry_name].name_id

			return managers.localization.text(slot8, managers.localization)
		end
	end
...
function LevelsTweakData:get_default_team_ID(type)
	...
	if lvl_tweak and lvl_tweak.default_teams and lvl_tweak.default_teams[type] then
		if lvl_tweak.teams[lvl_tweak.default_teams[type]] then
			...
			slot8 = Global.level_data.level_id

			debug_pause(slot4, "[LevelsTweakData:get_default_player_team_ID] Team not defined ", lvl_tweak.default_teams[type], "in")
		end
	end
...
function LevelsTweakData:get_team_setup()
	...
	if Application.editor(slot3) and managers.editor then
		slot5 = "Level Settings"
		slot5 = "simulation_level_id"
		lvl_tweak = self[managers.editor.layer(slot3, managers.editor).get_setting(slot3, managers.editor.layer(slot3, managers.editor))]
	else
...
function LevelsTweakData:get_default_team_IDs()
	...
	if not default_team_IDs then
		slot7 = "player"
		slot7 = "combatant"
		slot7 = "non_combatant"
		slot7 = "gangster"
		default_team_IDs = {
			player = self.get_default_team_ID(slot5, self),
			combatant = self.get_default_team_ID(slot5, self),
			non_combatant = self.get_default_team_ID(slot5, self),
			gangster = self.get_default_team_ID(slot5, self)
		}
	end
...
function LevelsTweakData:get_team_names_indexed()
	...
	if not teams_index then
		teams_index = {}
		local teams = self.get_team_setup(slot3)
		slot5 = teams

		for team_id, team_data in pairs(self) do
			slot11 = team_id

			table.insert(slot9, teams_index)
		end
...
function LevelsTweakData:get_team_index(team_id)
	local teams_index = self.get_team_names_indexed(slot3)
	slot5 = teams_index

	for index, test_team_id in ipairs(self) do
