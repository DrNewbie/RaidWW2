...
function IntelTweakData:get_control_video_by_path(path)
	...
	slot4 = self.categories.control_archive.items
...
function IntelTweakData:get_item_data(category_name, item_id)
	slot5 = self.categories[category_name].items

	for _, item_data in ipairs(slot4) do
		slot11 = item_id

		if tostring(slot9) == tostring(item_data.id) then
			return item_data
		end
	end
