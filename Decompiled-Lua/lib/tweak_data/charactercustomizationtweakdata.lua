...
function CharacterCustomizationTweakData:get_all_parts_indexed(part_type, nationality, owned)
	local result = {}
	slot8 = part_type
	local source_table = self.get_index_table(slot6, self)
	slot8 = source_table

	for index, part_key in ipairs(self) do
		result[index] = {}
		slot13 = self.customizations[part_key]
		local data_part = clone(slot12)
...
function CharacterCustomizationTweakData:get_all_parts(part_type)
	local result = {}
	slot6 = part_type
	local source_table = self.get_index_table(slot4, self)
	slot6 = source_table

	for index, part_key in ipairs(self) do
		result[part_key] = {}
		slot11 = self.customizations[part_key]
		local data_part = clone(slot10)
		data_part.key_name = part_key
...
function CharacterCustomizationTweakData:get_defaults()
	...
	slot4 = self.customizations

	for key, customization in pairs(slot3) do
		slot11 = customization

		if self._is_customization_default(slot8, self, key) then
...
function CharacterCustomizationTweakData:get_reward_loot_by_rarity(rarity)
	local customizations = {}
	slot5 = self.customizations

	for key, customization in pairs(slot4) do
		if customization.rarity == rarity and customization.droppable then
			slot11 = customization

			if self._is_customization_droppable(slot9, self) then
				slot11 = key

				table.insert(slot9, customizations)
			end
		end
	end
...
function CharacterCustomizationTweakData:get_droppable_customizations()
	...
	for key, customization in pairs(slot3) do
		slot10 = customization

		if self._is_customization_droppable(slot8, self) then
			slot10 = key

			table.insert(slot8, customizations)
		end
	end
...
function CharacterCustomizationTweakData:_is_customization_droppable(customization)
	...
	if customization.gold_price then
		slot4 = "[CharacterCustomizationTweakData][_is_customization_droppable] A customization is droppable and has a gold price at the same time: "
		slot7 = customization

		debug_pause(slot3, inspect(slot6))
	end

	if customization.dlc then
		slot5 = customization.dlc
...
function CharacterCustomizationTweakData:_is_customization_default(key, customization)
	...
	slot5 = tweak_data.dlc
	local unlocked_customizations = tweak_data.dlc.get_unlocked_customizations(slot4)
