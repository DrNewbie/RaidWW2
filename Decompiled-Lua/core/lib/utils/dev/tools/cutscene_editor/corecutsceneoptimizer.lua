...
-- WARNING: Error occurred during decompilation.
--   Code may be incomplete or incorrect.
slot2 = "core/lib/utils/dev/tools/cutscene_editor/CoreCutsceneExporter"
...
CoreCutsceneOptimizer.export_to_database = function (self, optimized_cutscene_path)
	...
	slot5 = optimized_cutscene_path
	local cutscene_name = managers.database.entry_name(slot3, managers.database)
	slot9 = cutscene_name .. "_%03i."
	slot7 = ".animation_blob"
	local part_path_pattern = string.gsub(managers.database, string.gsub(slot6, optimized_cutscene_path, cutscene_name .. "."), "%.cutscene$")
	slot6 = self
	local merged_animation = self._create_merged_animation(string.gsub(slot6, optimized_cutscene_path, cutscene_name .. "."))

	self._write_cutscene_xml(slot7, self, optimized_cutscene_path)

	slot8 = self
	slot13 = ".unit"

	self._write_cutscene_unit_xml(slot7, string.gsub(new_animation_blobs, optimized_cutscene_path, "%.cutscene$"))

	slot8 = managers.database

	managers.database.recompile(slot7)
...
CoreCutsceneOptimizer._write_animation_blobs = function (self, full_animation, part_path_pattern)
	...
	slot8 = full_animation
	local duration = full_animation.length(slot7)
	local start_time = 0
	local index = 0

	while start_time <= duration do
		slot12 = duration
		local end_time = math.min(slot10, start_time + self.ANIMATION_BLOB_PART_DURATION)
		slot15 = end_time
		local part = AnimationCutter.cut(start_time + self.ANIMATION_BLOB_PART_DURATION, AnimationCutter, full_animation, start_time)
		slot16 = per_bone_settings
		...
		slot18 = compressed_part

		self._write_animation_part(part_path_pattern, self, part_path, part)

		slot15 = animation_blob_names
		slot19 = part_path

		table.insert(part_path_pattern, managers.database.entry_path(part, managers.database))

		slot15 = compressed_part
...
CoreCutsceneOptimizer._write_animation_part = function (self, path, animation, compressed_animation)
	local platforms_to_export = {}
	local base_path = path
	slot8 = self.__compression_enabled

	for platform, _ in pairs(slot7) do
		slot14 = "%." .. platform .. "%."

		if string.find(slot12, base_path) then
			slot14 = platform

			table.insert(slot12, platforms_to_export)

			slot15 = "."
			base_path = string.gsub(slot12, base_path, "%." .. platform .. "%.")
		end
	end

	if #platforms_to_export == 0 then
		slot8 = self.__compression_enabled
		platforms_to_export = table.map_keys(slot7)
	end

	slot8 = platforms_to_export

	for _, platform in ipairs(slot7) do
		...
		save_func(slot14, AnimationCutter, (use_compressed and compressed_animation) or animation)
	end
